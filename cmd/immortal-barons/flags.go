...
func defineFlags(lang string, preDoor store.DoorConfig) *opts {
	return &opts{
		...
		name:            flag.String("name", defaultName(), i18n.T(lang, "play as `<name>` (only used with -local)")),
		...
		dataDir:         flag.String("data", "./data", i18n.T(lang, "the folder `<dir>` that holds the game data")),
		...
		boardID:         flag.String("board-id", "", i18n.T(lang, "this board's name in the league, `\"<name>\"`, for -ibbs-reset. Giving it skips the settings editor, for a member board that takes its rules from the Coordinator")),
		inboundDir:      flag.String("game-inbound", "", i18n.T(lang, "the game's own directory `<dir>` of packets waiting to be applied, for -ibbs-reset (default \"inbound\", under the data directory)")),
		outboundDir:     flag.String("game-outbound", "", i18n.T(lang, "the game's own directory `<dir>` of packets written for the other boards, for -ibbs-reset (default \"outbound\", under the data directory)")),
		importBoardCfg:  flag.String("import-bbs-cfg", "", i18n.T(lang, "take this board's name, league number, mailer and FTN directories from an original Barren Realms Elite BBS.CFG `<file>`, for -ibbs-reset")),
