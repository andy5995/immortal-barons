---
category: interbbs
in_game: true
order: 6
title: 'Террористические операции'
---

# Террористические операции

Террористические операции отправляют агентов против державы на другой
планете. Вы выбираете операцию, затем число агентов. Агенты тратятся в
момент отправки, чего бы они ни добились, а сам удар разрешается на планете
цели, когда до неё дойдёт пакет, — так что весть вернётся день спустя или
позже.

Защита новой державы останавливает любую операцию: защищённая держава не
получает никакого урона, и вы не можете отправить ни одной операции, пока
ваша собственная держава под защитой.

## Что делает каждая операция

Девять операций не взаимозаменяемы. Каждая бьёт по чему-то одному, и каждый
прошедший агент откусывает свой кусок:

- **Отправить шпиона** — не стоит цели ничего. Шпион, который проник,
  приносит домой сведения о её землях, нападении, обороне и золоте, а также
  о солдатах, истребителях, турелях, танках и боевом духе за ними. Вам
  сообщают об этом, когда шпион возвращается, и сведения попадают в Базу
  разведки, которая хранит пять последних отчётов по каждому государству и
  показывает, насколько различаются два последних. Поэтому шпион,
  отправленный до и после удара, показывает, что удар сделал. Когда вы
  наносите удар по государству, о котором у вас есть отчёт, его последний
  отчёт показывается, как только вы его выберете.
- **Бомбить разведку** — убивает несколько процентов их агентов, а ими они и
  защищаются, и нападают.
- **Деморализовать** — отнимает седьмую часть боевого духа армии.
- **Сеять раздор** — несколько процентов их солдат дезертируют.
- **Бомбить авиабазы** — сжигает часть их самолётов на земле.
- **Разжигать эмиграцию** — гонит людей прочь из державы.
- **Распространять пропаганду** — стоит им поддержки народа.
- **Бомбить склады еды** — самый непредсказуемый из девяти: может уничтожить
  почти треть запасов, а может почти ничего.
- **Диверсия в штабе** — отбрасывает их штаб назад на постоянную величину.

## Как выбрать

Подбирайте операцию под то, что вы задумали дальше. Если вы смягчаете
державу перед межпланетной атакой, берите те, что прореживают её оборону:
солдат, самолёты, боевой дух. Если вы изматываете державу, до которой пока
не дотянуться, берите те, что стоят ей роста: еда, люди, поддержка.

## Как агенты прорываются

За каждого агента бросается отдельный жребий, и агенты цели защищаются от
него.  Чем больше агентов у вашей державы, тем выше шанс каждого, но каждый
добавленный помогает меньше предыдущего. Когда у обеих держав агентов
поровну, проходит примерно двое агентов из пяти. Есть и небольшая
случайность в обе стороны, не зависящая ни от одной из сторон.

Поэтому чем больше агентов вы отправите, тем больше урона в среднем, но
вероятность прорыва для отдельного агента от этого не растёт. Договор о
Противодействии терроризму добавляет к обороне агентов партнёров, а
Разведывательный союз добавляет агентов ваших партнёров к вашим.
