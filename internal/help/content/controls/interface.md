---
title: Moving Through the Menus
category: controls
order: 1
in_game: true
---

# Moving Through the Menus

Press one key to choose. You do not press Enter to pick a menu item. Each item
shows its key in brackets, like `(1)`. Press that key and it runs at once.

"Quit" (the `0` key) leaves a turn menu (Spending, Attack, Covert, Trading) and
moves you to the next step of your turn. You cannot go back to that menu until
your next turn.

"Return" (the `0` key) leaves a side menu (like the Bank or the System Menu) and
takes you back to where you were.

The System Menu opens with the `*` key from the Spending menu. It holds extra
options like Preferences, Set Tax Rate, and Show Instructions.

If you turn on "Enter exits Buy menu" in Preferences, pressing Enter at a turn
menu chooses Quit for you.

Press `?` at a menu to open this help.
