---
category: interbbs
in_game: true
order: 2
title: 'Diplomacia Planetária'
---

# Diplomacia Planetária

Seu BBS mantém uma lista curta de como está com cada um dos outros
planetas. Cada um é marcado como **Inimigo**, **Nenhuma**, **Paz** ou
**Aliado**. Abra-a em Operações Interplanetárias, em **Lista
Diplomática**. Você também vê a marca de um planeta sempre que indica um, na
linha que diz "Nossas relações atuais com...".

Só o Coordenador do seu BBS pode mudar a lista. Ela fica no Menu do
Coordenador, em Alterar a Diplomacia.

## O que ela faz e o que não faz

A lista é um recado do seu Coordenador aos barões do seu próprio BBS. Ela
diz o que o seu BBS acha de um planeta. Ela quase não muda nada no jogo:

- Ela não impede um ataque. Você ainda pode atacar um planeta marcado como
  Aliado, e um planeta marcado como Inimigo ainda pode atacar você.
- Os outros planetas nunca a veem. Eles mantêm a lista deles, e ela pode
  dizer algo bem diferente a seu respeito.

Duas coisas usam a lista de verdade:

- **Comércio.** Você só pode comprar no mercado de outro planeta enquanto o
  seu BBS marcar aquele planeta como **Aliado**. Os dois BBSes precisam
  concordar: o seu tem de chamar o deles de Aliado para o seu lance sair, e
  o deles tem de chamar o seu de Aliado para atendê-lo. Se uma aliança
  acabar enquanto o seu lance está viajando, o seu ouro volta.
- **Mensagens.** Escrever para **Planetas Aliados** pelo menu de Mensagens
  IP envia para todo planeta que a lista marcar como Aliado.

Então peça ao seu Coordenador para marcar um planeta como Aliado antes de
planejar comerciar lá.

## Usando a lista

Leia a lista antes de empenhar forças. Seu Coordenador marca um planeta como
Inimigo depois de ver algo que você talvez não tenha visto — um ataque a um
vizinho, um golpe no seu próprio BBS, uma mensagem que acabou mal. É o aviso
mais barato que você vai receber.

Se você discordar de uma marca, diga. O Coordenador é eleito, e a lista
acompanha quem ocupa o cargo.
