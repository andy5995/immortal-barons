...
// The trading market is keyed by the OWNER HANDLE, so unlike the treaty rows it
// needs no name reuse to leak: the caller of a crushed realm logs back in, is
// onboarded a fresh realm under the same handle, and would find their dead
// realm's escrowed goods and unpaid sale gold waiting for them.
func TestRebornRealmHoldsNoMarketPositionOfItsPredecessor(t *testing.T) {
	...
	if err := w.BuyFromMarket(buyer, "owner", "Trooper", 400); err != nil {
		t.Fatalf("buy: %v", err)
	}
	if n, gold := w.MarketForSale("owner", "Trooper"), w.MarketProceeds["owner"]; n != 600 || gold != 400_000 {
	...
	if n := w.MarketForSale("owner", "Trooper"); n != 0 {
	...
	// handle, so try to buy off it before the caller comes back. The sale must be
	...
	_ = w.BuyFromMarket(buyer, "owner", "Trooper", 100)

	reborn := w.AddHuman("owner", "Reborn")
...
// Abdication frees the handle immediately — the caller can be back with a new
// realm the same day — so it must take the market position with it too.
func TestAbdicationDropsItsMarketPosition(t *testing.T) {
	...
	if err := w.BuyFromMarket(buyer, "q", "Tank", 4); err != nil {
		t.Fatalf("buy: %v", err)
	}
	if n, gold := w.MarketForSale("q", "Tank"), w.MarketProceeds["q"]; n != 6 || gold != 4_000 {
		t.Fatalf("the listing and sale did not take (listed=%d, proceeds=%d), so nothing below is tested", n, gold)
	}
