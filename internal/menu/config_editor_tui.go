...
const (
	...
	helpGameLength         = "Game days before this board's game ends. The realm with the most regions is crowned Planetary Master, every realm is removed, and a new game starts. 0 means the game never ends."
	helpIdleRemove         = "Days a realm may go unplayed before it is removed. A realm whose owner never played a turn is removed the next day. 0 never removes anything."
	helpStartDate          = "The date the game starts. Before it, players can join and play their first turns, but no new day begins. Blank starts the game right away."
	helpJoinDate           = "New players may not join after this date. Blank leaves joining open."
	helpInitialLand        = "Extra regions each new realm may buy when it is created, on top of its first day's allowance."
	helpLandPerDay         = "Regions added each day to what each realm may buy."
	helpInterest           = "Bank interest over 10 days: 50 is 5% a day, and the maximum (200) is 20% a day. It is paid in equal parts at the end of each turn a player takes. High rates let banking dominate the game, so a value near the minimum keeps play balanced."
	helpStdInvest          = "The investment return over 10 days when Steady Investment Rate is on: 35 is 3.5% a day, and the maximum (100) is 10% a day. When Steady Investment Rate is off, the return drifts each day with how much is invested, and the bank pulls it back when it falls below half of this value or rises above one and a half times it."
	helpSteadyInvest       = "When on, the investment return stays fixed at the Standard Investment Rate instead of drifting each day with how much is invested."
	...
	helpBuyMilitary        = "Whether players may buy military units. Yes: they may buy them freely. No: they must build them with industry. Limited is not built yet and works like Yes."
	...
	helpRegionCosts        = "How fast the price of a region rises for a realm with 300 or more regions. Smaller realms and realms under protection are not affected. None removes the extra rise; it does not make regions free."
	helpAttackDamage       = "How much of its force each side loses before it breaks off a battle: High, Medium, or Low. It applies to attacks on this board and on other boards."
	helpAttackRewards      = "How large a share of a beaten realm's regions the winner takes: High, Medium, or Low. Only regions change hands."
	helpSabre              = "How S3-Sabre missiles sent to other boards are aimed. User Select: the player sets the dial. Random: a random dial for each launch. Constant: always the dial set below. None: the missile cannot be used."
	helpSabreDial          = "Which dial Constant handling fires (0-10). Ignored by the other modes."
	helpMaxLocalAttacks    = "The most attacks a player may make on this board in one day. Regular attacks, nuclear, chemical and biological attacks, and attacks on pirates all count. A player can still make only one per turn. 0 means no daily limit beyond that."
	helpMaxAttacks         = "The most Individual Attack Forces a player may send to other boards in one day. 0 means no limit. Attacks on this board are limited by Max Local Attacks instead."
	...
	helpTerrorCosts        = "How much gold terrorist operations and the four bombing operations cost to launch."
	...
	helpLocalAttackScoring = "Whether winning a regular attack on a baron on this board earns score. The loser never loses score."
	helpDupeChecking       = "Whether a baron found playing on another board in the league is locked out here."
	helpMissileOps         = "Whether players may send nuclear, chemical and S3-Sabre missiles to other boards. Attacks on this board are not affected."
	...
	helpMaxRegions         = "The most regions a player may buy in one turn. It does not limit how many regions a player may own. 0 means no limit."
	helpMaxPlayers         = "The most players on this board, 1 to 25. 0 means as many as the planet has room for (25)."
	...
	helpLeagueNumber       = "The number this league runs under, 1-999. A board playing in a league needs one: left at 0 it takes every league's packets as its own, and the inter-BBS runs refuse to start. Set in bbs.cfg."
	...
	helpIdleTimeout        = "Disconnect a player after this many seconds with no keypress. The player is warned first. 0 never times out."
	helpIdleWarnings       = "How many times in a row a player may let the idle warning appear before pressing a key. After that, the next idle period disconnects without a warning."
)
...
func newConfigTUI(w *game.World) *configTUI {
	...
	t.addInt(econ, "Interest Rate", helpInterest, c.InterestRate, game.MinBankInterest, game.MaxBankInterest, func(c *game.Config, n int) { c.InterestRate = n })
	t.addInt(econ, "Standard Investment Rate", helpStdInvest, c.StdInvestRate, game.MinStdInvestRate, game.MaxStdInvestRate, func(c *game.Config, n int) { c.StdInvestRate = n })
	...
	t.addInt(caps, "Max Players Per BBS (0=unlimited)", helpMaxPlayers, c.MaxPlayers, 0, game.MaxPlayersPerBoard, func(c *game.Config, n int) { c.MaxPlayers = n })
	if ibbs {
