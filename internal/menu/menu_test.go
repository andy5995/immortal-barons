...
func TestTurnaroundLabelUnits(t *testing.T) {
	...
	cases := []struct {
		...
		// chosen from the raw value and the figure rounded afterward, so a
...
func TestTargetLettersDoNotShiftWhenRealmsBecomeUnpickable(t *testing.T) {
	...
	f := &fakeSession{keys: []rune("C")}
	name, chosen := pickAttackTarget(f, Term{UTF8: true}, rows, warTargets.prompts("Choose a target"))
...
func TestAnAlliedRealmsLetterSelectsNothing(t *testing.T) {
	...
	if name, chosen := pickAttackTarget(f, Term{UTF8: true}, rows, warTargets.prompts("Choose a target")); chosen {
